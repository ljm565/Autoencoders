...
* ### 모델 학습 조건 설정 (config.json)
    ...
    * ### 사용자 지정 데이터 사용 시
        * custom_data_proportion
            * 사용자 지정 데이터의 trainset/valset/testet 폴더가 다 따로 있을 시: [1]
            * 위의 데이터 중 두 데이터가 한 폴더에 있을 시 나눌 비율 설정(e.g. [0.7, 0.3]), 
            * 사용자 지정 데이터가 구분 없이 하나의 경로에 있을 때 데이터 나누는 비율 설정 예시: [0.7, 0.2, 0.1]
        * 
        <br><br>
